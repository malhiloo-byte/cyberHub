...
## 25. Service metadata preflight compatibility record

A separately authorized WSL localhost P3 invocation passed Scope, Target, binary identity, bounded process execution, and state parsing. It then rejected a standard Nmap `service` element because the prior allowlist accepted only `name`, `product`, and `version`. The live invocation was not retried; its redacted receipt was `NMAP_XML_INVALID` after 3144ms.

The offline contract now requires `name`, `conf`, and `method`. `conf` must be one of `0` through `10`; `method` must be `table` or `probed`. It conditionally accepts standard Nmap DTD attributes `version`, `product`, `extrainfo`, `tunnel`, `proto`, `rpcnum`, `lowver`, `highver`, `hostname`, `ostype`, `devicetype`, and `servicefp`, with nonempty bounded text fields, numeric checks, and closed enumerations for tunnel/proto. Standard child `cpe` is accepted only inside a service and only without attributes.

The bridge retains only `name`, `product`, and `version` in normalized output. Confidence, method, CPE, fingerprints, hostnames, and all other service metadata are validated then discarded. Fixtures prove acceptance of a fully populated standard service, rejection of missing/invalid required metadata and unknown attributes, plus existing DTD/XXE protection. The patch has **409 passing tests**, zero migrations, and no live execution. A fresh explicit P3 authorization is required before any retry.

## 26. No-findings completion contract

A later authorized localhost receipt passed process execution and the complete XML parser contract, then reached `ReconIngestionService` with zero observations. This is the expected representation when no allowed port is open. The generic ingestion service correctly rejects an empty ReconResult because it is designed for discovery persistence; treating that rejection as a scan failure at the localhost application boundary was an integration contradiction.

`NmapLocalhostScanService` now handles this exact no-findings outcome before calling the discovery bridge. It persists the running Task as `COMPLETED` using the bounded/redacted execution result, returns a receipt with `parsed_services=0`, `parsed_observations=0`, and zero inserted asset/observation/evidence counters, and deliberately creates no synthetic Asset, Observation, or Evidence row. This is not data loss: the Task receipt is the truthful provenance record that the constrained scan completed with no findings.

Offline integration tests prove `COMPLETED` state/version, zero persisted Recon/Evidence records, and `quick_check`/`foreign_key_check` integrity. The consolidation suite now has **410 passing tests**, zero migrations, zero live execution, and no change to the generic Module 1.1 ingestion invariant. Any new P3 is a separate explicit authorization.
